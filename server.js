...
`;
  ...
  // ==========================================================================
  // sh-quote: wrap in single quotes, escaping any embedded quote. Makes commands
  // safe for dirs with spaces and binaries at absolute paths.
  function shq(x) { return "'" + String(x).replace(/'/g, "'\\''") + "'"; }
  const RUNNERS = [
    { id: 'claude', bin: 'claude', flag: '-p', args: (brief) => ['-p', brief, '--permission-mode', 'acceptEdits'],
      runLine: (dir, bin) => 'cd ' + shq(dir) + ' && ' + shq(bin || 'claude') + ' -p "$(cat BRIEF.md)"' },
    { id: 'codex', bin: 'codex', flag: 'exec', args: (brief) => ['exec', brief],
  ...
  async function apiGetEnv(req, res) {
    ...
    const bins = {};
    for (const d of r.detected) bins[d.id] = d.bin; // absolute path -> shell-independent commands
    sendJSON(res, 200, { runners: r.detected.map((x) => x.id), bins, preferred: r.preferred, platform: process.platform });
  }
  ...
  async function runTask(task) {
    ...
    try {
      ...
      const runLine0 = rr0 ? rr0.runLine(runDir, rbin0 && rbin0.bin) : null;
  ...
  async function apiPostAutomate(req, res) {
    ...
    const result = await withScanJsonLock(async () => {
      ...
      const runLine = rr ? rr.runLine(playbookDir, rbin && rbin.bin) : null;
      const brief = renderBrief(finding, scan, playbookDir, runLine);
